...
game_characters = ['Grumpy Mutant', 'Monstrous Nether Gorilla', 'Vampsnake', 'Hidden Assassin Elephant']
...
def intro(char):
    ...
    print_pause("next morning in a thick, dank forest. Head spinning and ") 
    print_pause("fighting the urge to vomit, you stand and marvel at your new, ")
    print_pause("unfamiliar setting. The peace quickly fades when you hear a ")
    print_pause("grotesque sound emitting behind you. A " + char +"is ")
...
def option_rock(char): 
    print_pause("\nThe "+ char + " is stunned, but regains control. He begins")
    ...
    if choice in answer_A:
        ...
        print_pause("rock thrown did much damage. The rock flew well over the ")
...
def option_cave():
    ...
    print_pause("ominous. Before you fully enter, you notice a shiny sword on ")
    ...
    if choice in answer_A:
        ...
        print_pause(+ char +" can see very well in the dark, right? Not sure, but ")
        print_pause("I'm going with YES, so...\n\nYou died!")
        play_again()
    elif choice in answer_B:
        if sword > 0:
            print_pause("\nYou laid in wait. The shimmering sword attracted ")
            print_pause("the " +char +", which thought you were no match. As he walked ")
            print_pause("closer and closer, your heart beat rapidly. As the "+ char )
            print_pause("reached out to grab the sword, you pierced the blade into ")
...
def option_town():
    ...
    print_pause("sword lying in the mud. You quickly reach down and grab it, ")
    print_pause("but miss. You try to calm your heavy breathing as you hide ")
    print_pause("behind a delapitated building, waiting for the "+ char +" to come ")
    print_pause("charging around the corner. You notice a purple flower ")
    ...
    if flower > 0:
        ...
        print_pause("hoping it will stop the "+char+". It does! The "+char+" was looking ")
